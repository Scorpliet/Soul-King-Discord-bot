...
async def on_ready():
    ...
    modules = [
        "game", "helpmod", "music", "chat", "admin", "error_handle", "image",
        "mal"
    ]
...
async def on_message(message):
    ...
    await bot.process_commands(
        message
    )  #This line makes sure on_message wont take priority over a command function
    pchance = randrange(8)
    if message.content.lower().lower().startswith(
        ("hello brook", "hello soulking", "hi brook", "hey brook")):
        randomliststart = [
        ...
        if response == randomliststart[2]:
            ...
            await message.channel.send(response + message.author.name +
                                       random.choice(randomlistend))
            if pchance == 4:
                await message.channel.send(
                    random.choice(presponses) + random.choice(pemojies))
    elif message.content.lower().startswith(
        ("brook see my pants", ".see my pants", "brook see my panties")):
        Images = [
...
async def loadmem(ctx):
    ...
    if len(members) == 0:
        ...
        msg = await ctx.author.send(
            f"Spotted {len(members)} members in Ship {guild.name} successfully"
        )
...
@commands.check(bot_owner)
async def coup(ctx):
    role = await ctx.author.guild.create_role(
        name="Bois OverLord",
